package Week_7.TaskDemo.utils;

import java.io.*;
import java.net.Socket;

import static Week_7.TaskDemo.commons.SimpleHttpServer.HTML_FILE_PATH;
import static Week_7.TaskDemo.commons.SimpleHttpServer.JSON_FILE_PATH;
...
public class HttpHandler extends Thread{
    ...
    private void sendNewHtmlResponse(PrintWriter output) throws IOException {
        ...
        if(file.exists()){
            sendResponse(output,file, "test/html");
        }else {
            sendNewNotFoundResponse(output);
        }
    }
    ...
    private void sendNewNotFoundResponse(PrintWriter output) {
        ...
        output.println("Output-type: text/plain");
        output.println();
